...
1. Select the first 50 entries from the entry table that do not have an idiomacity score and have no entry_tags record with tag 'scrabble'.
2. For each entry, generate a prompt using the phrase_idiomacity_prompt.txt file. Send the prompt to Gemini (using GeminiWebAiProvider standard mode)
    ...
    while (true) {
      ...
      try {
        ...
        if (isGeminiTimeoutError(error)) {
          console.warn(`Gemini timeout processing idiomacity batch ${batchNumber}`);
          break;
        }
