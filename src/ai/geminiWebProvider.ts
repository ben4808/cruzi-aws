...
class GeminiWebSession {
  ...
  private async setupPage(): Promise<void> {
    ...
    console.log(
      this.login
        ? this.useWebshare
          ? 'Gemini web logged-in session ready (Flash, via Webshare)'
          : 'Gemini web logged-in session ready (Flash)'
        : this.useWebshare
          ? 'Gemini web guest session ready (Flash, via Webshare)'
          : 'Gemini web guest session ready (Flash)',
    );
  }
  ...
  private async generateOnce(prompt: string, attempt: number): Promise<string> {
    ...
    console.log(
      `Waiting up to ${GEMINI_GENERATION_TIMEOUT_MS / 1000}s for Gemini Flash response (attempt ${attempt})...`,
    );
    return waitForGeminiResponse(page, GEMINI_GENERATION_TIMEOUT_MS);
  }
          ...
          try {
            ...
            if (isInternetConnectionToastError(error)) {
              ...
              const reason = isDetachedFrameError(error)
                ? 'page frame detached'
                : `response timed out after ${GEMINI_GENERATION_TIMEOUT_MS / 1000}s`;
              console.warn(
